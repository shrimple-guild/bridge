...
export class ManualVerifyCommand implements SlashCommand {
	...
	static data = new SlashCommandBuilder()
		...
		.addUserOption((option) =>
			option.setRequired(true).setName("user").setDescription("The Discord user to verify.")
		)
	...
	async execute(interaction: ChatInputCommandInteraction<"cached">) {
		try {
			...
			if (username == null) {
				...
				await interaction.followUp({
					embeds: [
						statusEmbed(
							"success",
							`Verified \`${member.user.tag}\` without a Minecraft account.`
						)
					]
				})
				return
			...
			await interaction.followUp({
				embeds: [
					statusEmbed("success", `Verified \`${member.user.tag}\` as \`${username}\`.`)
				]
			})
		} catch (e) {
