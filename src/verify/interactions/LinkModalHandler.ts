...
export class LinkModalHandler implements IInteractionHandler<ModalSubmitInteraction> {
	...
	async handle(interaction: ModalSubmitInteraction<CacheType>): Promise<void> {
		...
		if (!this.verification || !this.hypixelAPI)
		...
		if (player.discordTag == null)
			throw new Error("This Hypixel account isn't linked to any Discord account.")
		if (player.discordTag.toLowerCase() != discordTag) {
			let errorString = `${username} is linked to \`${player.discordTag}\`.`
			if (interaction.user.discriminator == "0")
				errorString += `**Due to the Discord username update, you may need to update your linked Discord username in-game.**`
		...
		if (interaction.member instanceof GuildMember) {
			...
			await interaction.followUp({
				ephemeral: true,
				embeds: [statusEmbed("success", `Verified as \`${username}\`.`)]
			})
		} else {
			await interaction.followUp({
				ephemeral: true,
				embeds: [statusEmbed("failure", `Not a guild member? Weird (please report this)!`)]
			})
		}
	}
}
