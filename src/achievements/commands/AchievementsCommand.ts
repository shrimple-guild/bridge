	...
	static data = new SlashCommandBuilder()
		...
		.addSubcommand((builder) =>
			builder
				.setName("update")
				.setDescription(
					"Update your achievement roles using your linked Minecraft account."
				)
		)
		...
		if (!roleData.changed) {
			await interaction.reply({
				embeds: [
					simpleEmbed(
						"Updated roles",
						"No changes were made. Use **/achievements info** to view all achievement roles."
					)
				],
				ephemeral: true
			})
