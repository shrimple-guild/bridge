...
const bobberCount = 3
const rodList = ["GIANT_FISHING_ROD", "HELLFIRE_ROD", "ROD_OF_THE_SEA", "AUGER_ROD"]
...
export function getGuildRank(reqs: Requirements): GuildJoinStatus {
	...
	const canJoinXpLVL = config.joinRequirements.some((req) => {
		return reqs.skyblockLevel >= req.sbLevel && reqs.overflowFishingXp >= req.overflowFishingXp
	})
...
export async function getGuildRequirementResults(profile: SkyblockProfile) {
	...
	if (
		member.pets_data?.pets.find(
			(pet) => pet.type == "FLYING_FISH" && pet.tier == "MYTHIC" && pet.exp >= petXpRequired
		)
	) {
		...
		member.pets_data?.pets.find(
			(pet) =>
				pet.type == "FLYING_FISH" && pet.tier == "LEGENDARY" && pet.exp >= petXpRequired
		)
	) {
	...
	const trophyHunterLevels = ["none", "bronze", "silver", "gold", "diamond"] as const
	...
	reqs.magmaLordSets = magmaLordSets.map((set) => {
		return set
			.map((piece) => {
				...
				let btLevel = piece.tag?.ExtraAttributes.enchantments?.ultimate_bobbin_time
				if (btLevel) {
	...
	reqs.bobbinTime = magmaLordSets
		.map((set) =>
			set.reduce(
				(bobbin, cur) =>
					bobbin + (cur.tag?.ExtraAttributes.enchantments?.ultimate_bobbin_time ?? 0),
				0
			)
		)
	...
	reqs.statsCheck = setups.some((setup) => setup.scc >= 100 && setup.speed >= 350)
...
async function getItemsInInventories(member: APISkyblockMember): Promise<NBTItem[]> {
...
async function getItemsInInventory(inventory: string | undefined): Promise<NBTItem[]> {
	if (!inventory) return []
	const buffer = Buffer.from(inventory, "base64")
	const items = await nbt.parse(buffer).then((data) => nbt.simplify(data.parsed).i as any[])
	return items
}
...
function getPresumedEquipmentStats(items: NBTItem[]) {
	...
	for (let item of items) {
		...
		if (isFishingEquipment) {
			const rarity = item.tag?.ExtraAttributes?.rarity_upgrades == 1 ? nextRarity : baseRarity
			const data = {
				scc:
					getReforge(
			...
			const type = id == "RIFT_NECKLACE_OUTSIDE" ? "NECKLACE" : id.slice(id.indexOf("_") + 1)
			// @ts-ignore
...
async function getAllWardrobeSets(member: APISkyblockMember): Promise<NBTItem[][]> {
	const wardrobeContents = await getItemsInInventory(member.inventory?.wardrobe_contents?.data)
...
function getPetStats(member: APISkyblockMember) {
	...
	const ammonite = pets.find(
		(pet) => pet.type == "AMMONITE" && pet.tier == "LEGENDARY" && pet.exp >= petXpRequired
	)
	if (ammonite != null) {
...
function getMagmaLordSets(sets: NBTItem[][]): NBTItem[][] {
	return sets.filter((set) => {
		const ids: string[] = set.map((piece) => piece?.tag?.ExtraAttributes?.id ?? "")
...
function getArmorStats(set: NBTItem[], mythicFish: boolean) {
	return set.reduce(
		(last, piece) => {
			...
			let scc = 0
			const isMagmaLord = piece.tag?.ExtraAttributes?.id?.startsWith("MAGMA_LORD_")
			if (isMagmaLord) {
				const rarity =
					piece?.tag?.ExtraAttributes?.rarity_upgrades == 1 ? "mythic" : "legendary"
				scc =
					4.5 +
					(getReforge(
				...
				const sccLine = lore.find((line) => line.startsWith("§7Sea Creature Chance: §c+"))
				scc = parseFloat(sccLine?.slice(25, sccLine.indexOf("%")) ?? "0")
			}
			const bobbin: number =
				piece?.tag?.ExtraAttributes.enchantments?.ultimate_bobbin_time ?? 0
			const fishingSpeedLine = lore.find((line) => line.startsWith("§7Fishing Speed: §a+"))
...
async function getAccessoryStats(accessoryBag: string | undefined) {
	...
	const enrichments = accessories.reduce(
		(count, accessory) =>
			count +
			(Object.hasOwn(accessory.tag?.ExtraAttributes ?? {}, "talisman_enrichment") ? 1 : 0),
		0
	)
	const enrichmentScc = 0.3 * enrichments

	const fishAffinitySpeed = hasAccessory(accessories, "FISH_AFFINITY_TALISMAN") ? 10 : 0
	let agarimooSpeed = 0
...
function getHellfireRodStats(rod: any) {
	...
	const rarity = rod.tag.ExtraAttributes.rarity_upgrades == 1 ? "mythic" : "legendary"
	const reforgeScc =
		getReforge(rod.tag.ExtraAttributes.modifier, rarity, "sea_creature_chance") ?? 0
	const reforgeSpeed = getReforge(rod.tag.ExtraAttributes.modifier, rarity, "fishing_speed") ?? 0
	...
	const expertiseScc = (rod.tag.ExtraAttributes.enchantments?.expertise ?? 0) * 0.6
	const totalScc = baseScc + starScc + reforgeScc + anglerScc + expertiseScc
	const totalSpeed = baseSpeed + starSpeed + reforgeSpeed + attributeSpeed + piscarySpeed
	return {
		scc: totalScc,
		speed: totalSpeed
	}
