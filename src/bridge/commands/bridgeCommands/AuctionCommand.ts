...
const itemResults = (
	(await (await fetch(`https://api.hypixel.net/v2/resources/skyblock/items`)).json()) as {
		items: { id: string; name: string }[]
	}
).items

let itemApiNames = Object.fromEntries(
	itemResults.map((itemData) => {
		let aliases: string[] = []
		return [itemData.id, { id: itemData.id, name: itemData.name, aliases: aliases }]
	})
)

let itemRemappings = Object.fromEntries(
	Object.entries(auctionAliases).map(([id, aliases]) => {
...
remapped.forEach((product) => {
	fullExpandedNames.push({
		id: product.id,
		name: product.name,
		alias: product.name.trim().toUpperCase()
	})
	...
	closestAuctionProduct(phrase: string[]) {
		...
		if (perfectMatches.length > 1) {
			bestMatch = perfectMatches.sort((a, b) => {
				return (
					uppercase.filter((phrase) => b.alias.toUpperCase().includes(phrase)).length -
					uppercase.filter((phrase) => a.alias.toUpperCase().includes(phrase)).length
				)
			})[0]
		} else if (perfectMatches.length === 1) {
			bestMatch = perfectMatches[0]
		} else {
			bestMatch = expandedNames.sort(
				(a, b) => jaroDistance(joined, b.alias) - jaroDistance(joined, a.alias)
			)[0]
		}
...
;(async function updateBinCache() {
	try {
		...
		if (auctionResponse.status === 200) {
			cachedLowestBins = (await auctionResponse.json()) as {
				[id: string]: number
			}
