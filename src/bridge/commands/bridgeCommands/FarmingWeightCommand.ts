...
export class FarmingWeightCommand extends SimpleCommand {
	...
	async execute(args: string[]) {
		...
		const bestCollections = Object.entries(weight.collections)
			.sort(([crop1, collection1], [crop2, collection2]) => collection2 - collection1)
			.slice(0, 2)
			.map(([crop, weight]) => `${crop} (${formatNumber(weight, 2, false)})`)
			.join(", ")
