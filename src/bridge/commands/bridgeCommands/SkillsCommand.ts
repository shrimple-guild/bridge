...
export class SkillsCommand extends SimpleCommand {
	...
	async execute(args: string[]) {
		...
		if (!normal.reachedUserCap() && (hasOverflowCurve || normal.getMaxLevel() != normal.getLevel())) {
			messageComponents.push(`XP for ${overflow.getLevel() + 1}: ${formatNumber(overflow.getXpToNextLevel() ?? Infinity, 2, true)}`)
		}
		messageComponents.push(`XP past ${normal.getUserMaxLevel()}: ${formatNumber(normal.getCurrentXp(), 3, true)}`)
