...
export class TrophyFish {
  ...
  constructor(member: any) {
    ...
    this.fish = Object.fromEntries(
      Object.entries(fishNameMap).map(([apiName, fishName]) => [fishName, new Fish(apiName, trophyData)])
  ...
  get tier(): TrophyFishTier | undefined {
    return Object.values(this.fish).map(fish => fish.tier).reduce((prev, cur) => (
      (tierValue(prev) < tierValue(cur)) ? prev : cur
    ))
  }
