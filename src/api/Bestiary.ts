...
export class Bestiary {
  ...
  constructor(member: any) {
    ...
    for (const type of neuBestiaryData.types) {
      ...
      for (const typeIterable of typeIterator) {
        const islandObj: TypeData = { name: typeIterable.name, mobs: [], hasSubcategories: false, subcategories: [] }
        if (typeIterable.mobs) {
          for (const mob of typeIterable.mobs) {
            const cleanedName = stripColorCodes(mob.name)
            let mobObj = islandObj.mobs.find(islandMob => islandMob.name === cleanedName)
            if (!mobObj) {
              mobObj = {
                name: cleanedName,
                cap: mob.cap,
  ...
  getByIsland(island: string) {
    return this.data.find(data => data.name.toLowerCase() === island.toLowerCase() || data.name.toLowerCase().includes(island.toLowerCase()))
  }

  getByMob(mob: string) {
    for (const island of this.data) {
      const found = island.mobs.find(m => m.name.toLowerCase() === mob.toLowerCase() || m.name.toLowerCase().includes(mob.toLowerCase()))
