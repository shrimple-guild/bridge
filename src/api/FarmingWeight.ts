...
function collectionWeight(crops: Collections): Collections {
	const base = Object.fromEntries(
		Object.entries(crops).map(([crop, collection]) => [
			crop,
			collection / weights[crop as Crop]
		])
	)
...
function contestIsGold(contest: any): boolean {
	return (
		...
		contest.claimed_position <= contest.claimed_participants * 0.05 + 1
	)
}

function medalWeight(profileMember: any): number {
	const contests = profileMember.jacob2?.contests ?? {}
	const golds = (Object.values(contests) as any[]).reduce(
		(golds, contest) => golds + (contestIsGold(contest) ? 1 : 0),
		0
	)
...
function minionWeight(profile: any): number {
	const minions = (Object.values(profile.members) as any[]).flatMap(
		(member) => member.crafted_generators ?? []
	)
	const minionNames = [
		"WHEAT",
		"CARROT",
	...
	return minionNames.reduce((cum, cur) => cum + (minions.includes(cur + "_12") ? 5 : 0), 0)
}
